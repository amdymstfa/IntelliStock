...
class ProductServiceTest {
    ...
    void shouldCreateProductSuccessfully() {
        ...
        verify(productRepository).save(argThat(product ->
                product.getMargin().equals(new BigDecimal("300.00")) // تأكد أن الحساب صحيح
        ));
        verifyNoInteractions(productMapper.getClass().equals(EncryptionService.class) ? null : mock(EncryptionService.class));
    }
    ...
    @Test
    @DisplayName("Should update product and recalculate margin")
    ...
    @Test
    @DisplayName("Should get admin product response")
    void shouldGetAdminProductResponse() {
        // Arrange
        when(productRepository.findById(1L)).thenReturn(Optional.of(testProduct));
        ...
        verify(productMapper).toAdminResponse(testProduct);
    }
